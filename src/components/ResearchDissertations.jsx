...
const PAPERS = [
  {
    id: "paper1",
    type: "CO-AUTHOR",
    typeColor: "text-[#0a84ff]",
    pulse: true,
    dotColor: "bg-[#0a84ff]",
    title: "Dual-Channel CNN-LSTM with MFCC-Chroma Attention Fusion for Lung Sound Disease Classification",
    abstract: "Features novel attention convergence modeling using stethoscopic acoustic streams. Achieved a macro F1 of 0.4056 and 0.3394 on clinical patients in the Philippines.",
    link: "https://bitly.cx/lungs",
    tags: [
      "Data Preprocessing",
      "Model Development",
      "HINGA V1 Dataset"
  ...
  },
  {
    ...
    pulse: false,
    dotColor: "bg-[#0a84ff]",
    title: "Identifying Distinctive Acoustic Signatures of Respiratory Diseases: A PCA-and ICA-Based Exploration of the ICBHI 2017 Dataset",
    abstract: "Aims to enhance the interpretability of respiratory sound analysis, support future feature-selection strategies, and contribute to the development of more transparent and clinically reliable diagnostic systems.",
    link: "https://www.researchgate.net/publication/407118265_Identifying_Distinctive_Acoustic_Signatures_of_Respiratory_Diseases_A_PCA-and_ICA-Based_Exploration_of_the_ICBHI_2017_Dataset",
    tags: [
      "Principal Author",
  ...
  },
  {
    ...
    pulse: false,
    dotColor: "bg-[#0a84ff]",
    title: "Virtual Machines vs. Container Technology: Trade-offs, Challenges, and Hybrid Solutions in Modern Data Centers",
    abstract: "Seeks to critically examine whether containers can meet the operational and security requirements of heterogeneous environments, or whether a hybrid deployment strategy combining virtual machines and containers is necessary to achieve an optimal balance.",
    link: "https://www.researchgate.net/publication/407117946_Virtual_Machines_vs_Container_Technology_Trade-offs_Challenges_and_Hybrid_Solutions_in_Modern_Data_Centers",
    tags: [
      "Virtualization",
      "Container Tech",
      "Hybrid Deployments"
  ...
  },
  {
    ...
    pulse: false,
    dotColor: "bg-[#0a84ff]",
    title: "MFCC-Chroma Fusion and Demographic-Aware Random Forest for Lung Sound Disease Classification",
    abstract: "Synthesizes chronological acoustics alongside patient metadata profiles (age, sex) to distinguish restrictive vs. obstructive signals.",
    link: "https://www.researchgate.net/publication/391980454_MFCC-Chroma_Fusion_using_a_Dual-Channel_CNN-LSTM_and_Demographic-Aware_Random_Forest_for_Lung_Sound_Disease_Classification",
    tags: [
      "Seed Study",
      "Demographic-Aware RF",
      "Acoustic Fusion"
    ]
  },
  {
    ...
    pulse: false,
    dotColor: "bg-[#0a84ff]",
    title: "Automaton-Based Framework for Validating and Extracting Information from Vehicle Plate Numbers in the Philippines",
    abstract: "A finite state automaton that validates Philippine plate numbers of motorcycles and 4-wheel vehicles.",
    link: "https://www.researchgate.net/publication/380362488_Automaton-Based_Framework_for_Validating_and_Extracting_Information_from_Vehicle_Plate_Numbers_in_the_Philippines",
    tags: [
      "Automata Theory",
      "Plate Validation",
      "LTO Standards"
    ...
    >
      <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-7 space-y-5 max-h-[500px] overflow-y-auto pr-1.5 scrollbar-thin">
            {PAPERS.map((paper) => (
              <div 
                key={paper.id}
                className="bg-[#050505] border border-neutral-800 p-4 sm:p-5 rounded-2xl transition-all duration-300 hover:border-[#0a84ff]/40 hover:shadow-[0_0_25px_rgba(10,132,255,0.06)] hover:-translate-y-1 hover:scale-[1.01] hover:bg-[#080808] group/paper cursor-pointer"
              >
                <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-neutral-950 border border-neutral-800 text-[9px] font-mono font-black ${paper.typeColor} tracking-widest group-hover/paper:border-[#0a84ff]/30 transition-colors`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${paper.dotColor} ${paper.pulse ? 'animate-pulse' : ''}`} />
                  {paper.type}
                </div>
                <h4 className="font-display font-extrabold text-white text-base mt-2 leading-snug group-hover/paper:text-[#0a84ff] transition-colors font-display">
                  {paper.title}
                </h4>
                ...
                <div className="flex flex-wrap items-center gap-2.5 mt-4 font-mono text-[10px]">
                  {paper.tags.map((tag, idx) => (
                    <span 
                      key={idx} 
                      className="text-[#0a84ff] bg-[#0a84ff]/5 border border-[#0a84ff]/10 px-2 py-0.5 rounded font-mono font-extrabold text-[9px] tracking-wide uppercase"
                    >
                      {tag}
                    </span>
                  ))}
                  <a 
                    ...
                    target="_blank" 
                    rel="noreferrer" 
                    className="text-blue-500 font-extrabold hover:underline flex items-center gap-1 ml-auto cursor-pointer"
                  >
          ...
          <div className="lg:col-span-5 bg-black border border-neutral-800 rounded-2xl p-5 flex flex-col gap-4 relative overflow-hidden min-h-[460px]">
            ...
            <div className="absolute inset-0 bg-[#07070a]/92 backdrop-blur-md z-20 flex flex-col items-center justify-center text-center p-6 sm:p-8 space-y-4">
              <div className="w-12 h-12 rounded-2xl bg-blue-500/10 border border-blue-500/30 flex items-center justify-center text-[#0a84ff] animate-pulse">
                <Settings className="w-6 h-6 animate-spin" />
              </div>
              
              <div className="space-y-1.5">
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-500/10 text-[#0a84ff] border border-blue-500/20 text-[10px] font-mono font-black tracking-widest uppercase">
              ...
              <div className="w-full max-w-[200px] bg-neutral-950 border border-neutral-900 rounded-lg p-2 font-mono text-[8px] text-neutral-600 flex items-center justify-between">
                ...
                <span className="text-[#0a84ff] font-bold">87.4% COMPLETED</span>
              </div>
            </div>
              ...
              {['copd', 'pneumonia', 'asthma', 'urti', 'bronchitis', 'crackle', 'healthy'].map((sig) => (
                <button
                  ...
                  className={`px-2.5 py-1.5 text-center font-mono text-[10px] uppercase font-bold rounded-lg border transition-all cursor-pointer ${
                    selectedSample === sig
                      ? 'bg-gradient-to-r from-[#00d2ff] to-[#0a84ff] text-white border-transparent font-extrabold shadow-md'
            ...
            <button
              ...
              className={`w-full py-2.5 rounded-xl font-mono text-xs font-extrabold uppercase transition-all tracking-widest flex items-center justify-center gap-2 cursor-pointer ${
                isAnalyzing
                  ...
                  : 'bg-gradient-to-r from-[#00d2ff] to-[#0a84ff] text-white hover:opacity-95 shadow-lg shadow-blue-600/25 hover:scale-[1.01]'
              }`}
            >
